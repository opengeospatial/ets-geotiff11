...
As entry point the Landing Page of the GeoTIFF 1.1 is expected.
...
One of the build artifacts is an "all-in-one" JAR file that includes the test suite and all of 
its dependencies (located in the "target" directory); this makes it very easy to execute the test
suite in a command shell:

`java -jar ets-geotiff11-0.1-SNAPSHOT-aio.jar [-o|--outputDir $TMPDIR] [test-run-props.xml]`
