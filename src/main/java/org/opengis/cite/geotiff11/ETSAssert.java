...
import org.opengis.cite.geotiff11.util.NamespaceBindings;
...
public class ETSAssert {
	 ...
	 *
	 * <ul>
	 * <li>ows: {@value org.opengis.cite.geotiff11.Namespaces#OWS}</li>
	 * <li>xlink: {@value org.opengis.cite.geotiff11.Namespaces#XLINK}</li>
	 * <li>gml: {@value org.opengis.cite.geotiff11.Namespaces#GML}</li>
	 * </ul>
	 ...
	 */
	/*
	 * seems unused public static void assertExceptionReport(Response rsp, String
	 * exceptionCode, String locator) { Assert.assertEquals(rsp.getStatus(),
	 * Response.Status.BAD_REQUEST.getStatusCode(),
	 * ErrorMessage.get(ErrorMessageKeys.UNEXPECTED_STATUS)); Document doc =
	 * rsp.getEntity(Document.class); String expr =
	 * String.format("//ows:Exception[@exceptionCode = '%s']", exceptionCode); NodeList
	 * nodeList = null; try { nodeList = XMLUtils.evaluateXPath(doc, expr, null); } catch
	 * (XPathExpressionException xpe) { // won't happen }
	 * Assert.assertTrue(nodeList.getLength() > 0, "Exception not found in response: " +
	 * expr); if (null != locator && !locator.isEmpty()) { Element exception = (Element)
	 * nodeList.item(0); String locatorValue =
	 * exception.getAttribute("locator").toLowerCase();
